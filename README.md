...
At game-build time the extension option `sdkPath` (GMFirebase, in the IDE) must point at the same unpacked root - one root for every target: the Android build reads the messaging AAR from it (the SDK's proguard rules ship inside the extension and are injected into the Android project), the iOS build stages the xcframeworks from it, and the Windows build copies `libs/windows/google_analytics.dll` from it beside the executable when the file is present (macOS and Linux link the SDK statically and stage nothing). It defaults to `../Firebase_sdk`, relative to the project folder, which is that location for the demo project. The two credential options default the same way: `jsonFile` to `../Firebase_private/google-services.json` (the Android file, which the desktop builds copy beside the executable as well) and `plistFile` to `../Firebase_private/GoogleService-Info.plist`; both folders are ignored by git, so put your own Firebase console files there or point the options elsewhere.
