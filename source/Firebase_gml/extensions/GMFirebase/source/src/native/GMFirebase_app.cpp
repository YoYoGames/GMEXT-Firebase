...
uint64_t firebase_app_initialize_with_options(const gm_structs::FirebaseAppOptions& options_value, std::string_view name)
{
	...
	if (!app)
	{
		setFirebaseLastError(GM_FB_ERROR_NOT_INITIALIZED, "firebase_app_initialize_with_options: Firebase App::Create returned null");
		return 0;
	}
#endif
...
uint64_t firebase_app_initialize_from_json(std::string_view json_config, std::string_view name)
{
	...
	if (!app)
	{
		setFirebaseLastError(GM_FB_ERROR_NOT_INITIALIZED, "firebase_app_initialize_from_json: Firebase App::Create returned null");
		return 0;
	}
#endif
...
std::string firebase_app_handle_get_name(uint64_t app_ref)
{
	...
	return app ? std::string(app->name()) : std::string();
}
