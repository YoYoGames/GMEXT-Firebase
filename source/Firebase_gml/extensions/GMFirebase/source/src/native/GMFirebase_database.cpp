...
uint64_t firebase_database_query_order_by_child(uint64_t ref, std::string_view path)
{
	return query_order_by_child(resolve_db_query(ref), path);
}
uint64_t firebase_database_query_order_by_key(uint64_t ref)
{
	return query_order_by_key(resolve_db_query(ref));
}
uint64_t firebase_database_query_order_by_value(uint64_t ref)
{
	return query_order_by_value(resolve_db_query(ref));
}
uint64_t firebase_database_query_order_by_priority(uint64_t ref)
{
	return query_order_by_priority(resolve_db_query(ref));
}
uint64_t firebase_database_query_start_at(uint64_t ref, const gm::wire::GMValue& order_value)
{
	return query_start_at(resolve_db_query(ref), order_value);
}
uint64_t firebase_database_query_start_at_key(uint64_t ref, const gm::wire::GMValue& order_value, std::string_view child_key)
{
	return query_start_at_key(resolve_db_query(ref), order_value, child_key);
}
uint64_t firebase_database_query_end_at(uint64_t ref, const gm::wire::GMValue& order_value)
{
	return query_end_at(resolve_db_query(ref), order_value);
}
uint64_t firebase_database_query_end_at_key(uint64_t ref, const gm::wire::GMValue& order_value, std::string_view child_key)
{
	return query_end_at_key(resolve_db_query(ref), order_value, child_key);
}
uint64_t firebase_database_query_equal_to(uint64_t ref, const gm::wire::GMValue& order_value)
{
	return query_equal_to(resolve_db_query(ref), order_value);
}
uint64_t firebase_database_query_equal_to_key(uint64_t ref, const gm::wire::GMValue& order_value, std::string_view child_key)
{
	return query_equal_to_key(resolve_db_query(ref), order_value, child_key);
}
uint64_t firebase_database_query_limit_to_first(uint64_t ref, double limit)
{
	return query_limit_to_first(resolve_db_query(ref), limit);
}
uint64_t firebase_database_query_limit_to_last(uint64_t ref, double limit)
{
	return query_limit_to_last(resolve_db_query(ref), limit);
}
uint64_t firebase_database_query_get_reference(uint64_t ref)
{
	return query_get_reference(resolve_db_query(ref));
}
bool firebase_database_query_set_keep_synchronized(uint64_t ref, bool keep_sync)
{
	return query_set_keep_synchronized(resolve_db_query(ref), keep_sync);
}
bool firebase_database_query_is_valid(uint64_t ref)
{
	return query_is_valid(resolve_db_query(ref));
}
FirebaseError firebase_database_query_get_value(uint64_t ref, const std::optional<gm::wire::GMFunction>& callback)
{
	return query_get_value(resolve_db_query(ref), "firebase_database_query_get_value", callback);
}
uint64_t firebase_database_query_add_value_listener(uint64_t ref, const std::optional<gm::wire::GMFunction>& on_value_changed, const std::optional<gm::wire::GMFunction>& on_cancelled)
{
	return query_add_value_listener(resolve_db_query(ref), on_value_changed, on_cancelled);
}
bool firebase_database_query_remove_value_listener(uint64_t ref, uint64_t listener_ref)
{
	return query_remove_value_listener(resolve_db_query(ref), listener_ref);
}
bool firebase_database_query_remove_all_value_listeners(uint64_t ref)
{
	return query_remove_all_value_listeners(resolve_db_query(ref));
}
...
bool firebase_database_query_remove_child_listener(uint64_t ref, uint64_t listener_ref)
{
	return query_remove_child_listener(resolve_db_query(ref), listener_ref);
}
bool firebase_database_query_remove_all_child_listeners(uint64_t ref)
{
	return query_remove_all_child_listeners(resolve_db_query(ref));
}
...
uint64_t firebase_database_get_instance_for_app(uint64_t app_ref)
{
	...
	if (!db || init_result != firebase::kInitResultSuccess)
	{
		setFirebaseLastError(GM_FB_ERROR_NOT_INITIALIZED, firebaseInitResultMessage("failed to get Realtime Database instance for app", init_result));
		return 0;
	}
	return registerFirebasePointer(db, GM_FB_TYPE_DATABASE);
}
...
uint64_t firebase_database_get_instance_for_app_url(uint64_t app_ref, std::string_view url)
{
	...
	if (!db || init_result != firebase::kInitResultSuccess)
	{
		setFirebaseLastError(GM_FB_ERROR_NOT_INITIALIZED, firebaseInitResultMessage("failed to get Realtime Database instance for app/url", init_result));
		return 0;
	}
	return registerFirebasePointer(db, GM_FB_TYPE_DATABASE);
}
