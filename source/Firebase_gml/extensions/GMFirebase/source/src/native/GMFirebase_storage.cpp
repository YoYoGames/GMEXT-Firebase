...
std::map<uint32_t, firebase::storage::StorageReference> g_storage_ref_map;
uint32_t g_storage_ref_index = 0;

std::map<uint32_t, firebase::storage::Metadata> g_storage_metadata_map;
uint32_t g_storage_metadata_index = 0;

std::map<uint32_t, firebase::storage::Controller> g_storage_controller_map;
uint32_t g_storage_controller_index = 0;

std::map<uint32_t, firebase::storage::StorageListResult> g_storage_list_result_map;
uint32_t g_storage_list_result_index = 0;

uint64_t registerStorageReference(const firebase::storage::StorageReference& ref)
{
	return packFirebaseRef(registerFirebaseValue(ref, g_storage_ref_index, g_storage_ref_map), GM_FB_TYPE_STORAGE_REF);
}

uint64_t registerStorageMetadata(const firebase::storage::Metadata& metadata)
{
	return packFirebaseRef(registerFirebaseValue(metadata, g_storage_metadata_index, g_storage_metadata_map), GM_FB_TYPE_STORAGE_METADATA);
}

uint64_t registerStorageListResult(const firebase::storage::StorageListResult& result)
{
	return packFirebaseRef(registerFirebaseValue(result, g_storage_list_result_index, g_storage_list_result_map), GM_FB_TYPE_STORAGE_LIST_RESULT);
}

namespace
{
	firebase::storage::Storage* resolveStorage(uint64_t storage_ref)
	{
		firebase::storage::Storage* storage = nullptr;
		validate_fb_ref_ptr(storage_ref, GM_FB_TYPE_STORAGE, firebase::storage::Storage, storage);
		return storage;
	}
	...
	firebase::storage::StorageReference* resolveStorageRef(uint64_t ref)
	{
		...
		validate_fb_ref_map(ref, GM_FB_TYPE_STORAGE_REF, firebase::storage::StorageReference, g_storage_ref_map, out);
		return out;
	}
	...
	firebase::storage::Metadata* resolveMetadata(uint64_t ref)
	{
		...
		validate_fb_ref_map(ref, GM_FB_TYPE_STORAGE_METADATA, firebase::storage::Metadata, g_storage_metadata_map, out);
		return out;
	}
	...
	firebase::storage::Controller* resolveController(uint64_t ref)
	{
		...
		validate_fb_ref_map(ref, GM_FB_TYPE_STORAGE_CONTROLLER, firebase::storage::Controller, g_storage_controller_map, out);
		return out;
	}
	...
	firebase::storage::StorageListResult* resolveListResult(uint64_t ref)
	{
		...
		validate_fb_ref_map(ref, GM_FB_TYPE_STORAGE_LIST_RESULT, firebase::storage::StorageListResult, g_storage_list_result_map, out);
		return out;
	}
...
uint64_t storage_get_instance_with_url(std::string_view url)
{
	firebase::App* app = getFirebaseApp();
	if (app == nullptr)
	{
...
std::string storage_url(uint64_t storage_ref)
{
	firebase::storage::Storage* storage = resolveStorage(storage_ref);
	if (storage == nullptr) return std::string();
	return storage->url();
}

uint64_t storage_get_reference(uint64_t storage_ref)
{
	firebase::storage::Storage* storage = resolveStorage(storage_ref);
...
uint64_t storage_get_reference_path(uint64_t storage_ref, std::string_view path)
{
	firebase::storage::Storage* storage = resolveStorage(storage_ref);
	if (storage == nullptr) return 0;
...
uint64_t storage_get_reference_from_url(uint64_t storage_ref, std::string_view url)
{
	firebase::storage::Storage* storage = resolveStorage(storage_ref);
	if (storage == nullptr) return 0;
...
double storage_max_download_retry_time(uint64_t storage_ref)
{
	firebase::storage::Storage* storage = resolveStorage(storage_ref);
...
void storage_set_max_download_retry_time(uint64_t storage_ref, double seconds)
{
	firebase::storage::Storage* storage = resolveStorage(storage_ref);
...
double storage_max_upload_retry_time(uint64_t storage_ref)
{
	firebase::storage::Storage* storage = resolveStorage(storage_ref);
	if (storage == nullptr) return 0.0;
...
void storage_set_max_upload_retry_time(uint64_t storage_ref, double seconds)
{
	firebase::storage::Storage* storage = resolveStorage(storage_ref);
	if (storage == nullptr) return;
...
double storage_max_operation_retry_time(uint64_t storage_ref)
{
	firebase::storage::Storage* storage = resolveStorage(storage_ref);
	if (storage == nullptr) return 0.0;
...
void storage_set_max_operation_retry_time(uint64_t storage_ref, double seconds)
{
	firebase::storage::Storage* storage = resolveStorage(storage_ref);
	if (storage == nullptr) return;
...
void storage_use_emulator(uint64_t storage_ref, std::string_view host, double port)
{
	firebase::storage::Storage* storage = resolveStorage(storage_ref);
	if (storage == nullptr) return;
...
uint64_t storage_ref_child(uint64_t ref, std::string_view path)
{
...
uint64_t storage_ref_get_parent(uint64_t ref)
{
	firebase::storage::StorageReference* self = resolveStorageRef(ref);
	if (self == nullptr) return 0;
...
std::string storage_ref_bucket(uint64_t ref)
{
	firebase::storage::StorageReference* self = resolveStorageRef(ref);
...
std::string storage_ref_full_path(uint64_t ref)
{
	firebase::storage::StorageReference* self = resolveStorageRef(ref);
	if (self == nullptr) return std::string();
...
std::string storage_ref_name(uint64_t ref)
{
	firebase::storage::StorageReference* self = resolveStorageRef(ref);
	if (self == nullptr) return std::string();
	return self->name();
...
double storage_ref_is_valid(uint64_t ref)
{
	firebase::storage::StorageReference* self = resolveStorageRef(ref);
...
uint64_t storage_ref_storage(uint64_t ref)
{
	firebase::storage::StorageReference* self = resolveStorageRef(ref);
	if (self == nullptr) return 0;
...
double storage_ref_delete(uint64_t ref, const std::optional<GMFunction>& callback)
{
	firebase::storage::StorageReference* self = resolveStorageRef(ref);
	if (self == nullptr) return 0.0;
...
double storage_ref_get_download_url(uint64_t ref, const std::optional<GMFunction>& callback)
{
	firebase::storage::StorageReference* self = resolveStorageRef(ref);
	if (self == nullptr) return 0.0;
...
double storage_ref_get_metadata(uint64_t ref, const std::optional<GMFunction>& callback)
{
	firebase::storage::StorageReference* self = resolveStorageRef(ref);
	if (self == nullptr) return 0.0;
...
// callback(error_code: real, error_message: string, metadata_ref: uint64)
double storage_ref_update_metadata(uint64_t ref, uint64_t metadata_ref, const std::optional<GMFunction>& callback)
{
	firebase::storage::StorageReference* self = resolveStorageRef(ref);
	if (self == nullptr) return 0.0;
...
double storage_ref_put_bytes(uint64_t ref, GMBuffer data, uint64_t metadata_ref,
...
// callback(error_code: real, error_message: string, metadata_ref: uint64)
double storage_ref_put_file(uint64_t ref, std::string_view local_path, uint64_t metadata_ref,
	const std::optional<GMFunction>& progress_callback, uint64_t controller_ref,
	const std::optional<GMFunction>& callback)
{
	firebase::storage::StorageReference* self = resolveStorageRef(ref);
...
double storage_ref_get_bytes(uint64_t ref, GMBuffer data,
	const std::optional<GMFunction>& progress_callback, uint64_t controller_ref,
	const std::optional<GMFunction>& callback)
{
	firebase::storage::StorageReference* self = resolveStorageRef(ref);
...
// callback(error_code: real, error_message: string, bytes_read: real)
double storage_ref_get_file(uint64_t ref, std::string_view local_path,
	const std::optional<GMFunction>& progress_callback, uint64_t controller_ref,
	const std::optional<GMFunction>& callback)
{
	firebase::storage::StorageReference* self = resolveStorageRef(ref);
...
double storage_ref_list(uint64_t ref, double max_results, std::string_view page_token, const std::optional<GMFunction>& callback)
{
	firebase::storage::StorageReference* self = resolveStorageRef(ref);
	if (self == nullptr) return 0.0;
...
uint64_t storage_metadata_create()
{
	return registerStorageMetadata(firebase::storage::Metadata());
}

double storage_metadata_release(uint64_t ref)
{
	firebase::storage::Metadata* self = resolveMetadata(ref);
	if (self == nullptr) return 0.0;
	return unregisterFirebaseValue(gm_fb_ref_id(ref), g_storage_metadata_map) ? 1.0 : 0.0;
}

double storage_metadata_is_valid(uint64_t ref)
{
	firebase::storage::Metadata* self = resolveMetadata(ref);
	if (self == nullptr) return 0.0;
	return self->is_valid() ? 1.0 : 0.0;
...
std::string storage_metadata_bucket(uint64_t ref)
{
	firebase::storage::Metadata* self = resolveMetadata(ref);
	if (self == nullptr) return std::string();
...
std::string storage_metadata_cache_control(uint64_t ref)
{
	firebase::storage::Metadata* self = resolveMetadata(ref);
	if (self == nullptr) return std::string();
...
void storage_metadata_set_cache_control(uint64_t ref, std::string_view value)
{
	firebase::storage::Metadata* self = resolveMetadata(ref);
...
std::string storage_metadata_content_disposition(uint64_t ref)
{
	firebase::storage::Metadata* self = resolveMetadata(ref);
	if (self == nullptr) return std::string();
...
void storage_metadata_set_content_disposition(uint64_t ref, std::string_view value)
{
	firebase::storage::Metadata* self = resolveMetadata(ref);
	if (self == nullptr) return;
...
std::string storage_metadata_content_encoding(uint64_t ref)
{
	firebase::storage::Metadata* self = resolveMetadata(ref);
	if (self == nullptr) return std::string();
...
void storage_metadata_set_content_encoding(uint64_t ref, std::string_view value)
{
	firebase::storage::Metadata* self = resolveMetadata(ref);
	if (self == nullptr) return;
...
std::string storage_metadata_content_language(uint64_t ref)
{
	firebase::storage::Metadata* self = resolveMetadata(ref);
	if (self == nullptr) return std::string();
...
void storage_metadata_set_content_language(uint64_t ref, std::string_view value)
{
	firebase::storage::Metadata* self = resolveMetadata(ref);
	if (self == nullptr) return;
...
std::string storage_metadata_content_type(uint64_t ref)
{
	firebase::storage::Metadata* self = resolveMetadata(ref);
	if (self == nullptr) return std::string();
...
void storage_metadata_set_content_type(uint64_t ref, std::string_view value)
{
	firebase::storage::Metadata* self = resolveMetadata(ref);
	if (self == nullptr) return;
...
double storage_metadata_creation_time(uint64_t ref)
{
	firebase::storage::Metadata* self = resolveMetadata(ref);
	if (self == nullptr) return 0.0;
...
double storage_metadata_generation(uint64_t ref)
{
	firebase::storage::Metadata* self = resolveMetadata(ref);
	if (self == nullptr) return 0.0;
...
double storage_metadata_metadata_generation(uint64_t ref)
{
	firebase::storage::Metadata* self = resolveMetadata(ref);
	if (self == nullptr) return 0.0;
...
std::string storage_metadata_name(uint64_t ref)
{
	firebase::storage::Metadata* self = resolveMetadata(ref);
	if (self == nullptr) return std::string();
...
std::string storage_metadata_path(uint64_t ref)
{
	firebase::storage::Metadata* self = resolveMetadata(ref);
	if (self == nullptr) return std::string();
...
uint64_t storage_metadata_get_reference(uint64_t ref)
{
	firebase::storage::Metadata* self = resolveMetadata(ref);
	if (self == nullptr) return 0;
...
double storage_metadata_size_bytes(uint64_t ref)
{
	firebase::storage::Metadata* self = resolveMetadata(ref);
	if (self == nullptr) return 0.0;
...
double storage_metadata_updated_time(uint64_t ref)
{
	firebase::storage::Metadata* self = resolveMetadata(ref);
	if (self == nullptr) return 0.0;
...
double storage_metadata_custom_metadata_count(uint64_t ref)
{
	firebase::storage::Metadata* self = resolveMetadata(ref);
	if (self == nullptr) return 0.0;
...
std::string storage_metadata_custom_metadata_key_at(uint64_t ref, double index)
{
	firebase::storage::Metadata* self = resolveMetadata(ref);
	if (self == nullptr) return std::string();
	std::map<std::string, std::string>* custom = self->custom_metadata();
...
std::string storage_metadata_get_custom_metadata(uint64_t ref, std::string_view key)
{
	firebase::storage::Metadata* self = resolveMetadata(ref);
	if (self == nullptr) return std::string();
	std::map<std::string, std::string>* custom = self->custom_metadata();
...
void storage_metadata_set_custom_metadata(uint64_t ref, std::string_view key, std::string_view value)
{
	firebase::storage::Metadata* self = resolveMetadata(ref);
	if (self == nullptr) return;
	std::map<std::string, std::string>* custom = self->custom_metadata();
...
uint64_t storage_controller_create()
{
	return packFirebaseRef(registerFirebaseValue(firebase::storage::Controller(), g_storage_controller_index, g_storage_controller_map), GM_FB_TYPE_STORAGE_CONTROLLER);
}

double storage_controller_release(uint64_t ref)
{
	firebase::storage::Controller* self = resolveController(ref);
	if (self == nullptr) return 0.0;
	return unregisterFirebaseValue(gm_fb_ref_id(ref), g_storage_controller_map) ? 1.0 : 0.0;
}

double storage_controller_is_valid(uint64_t ref)
{
	firebase::storage::Controller* self = resolveController(ref);
	if (self == nullptr) return 0.0;
	return self->is_valid() ? 1.0 : 0.0;
...
double storage_controller_pause(uint64_t ref)
{
	firebase::storage::Controller* self = resolveController(ref);
	if (self == nullptr) return 0.0;
...
double storage_controller_resume(uint64_t ref)
{
	firebase::storage::Controller* self = resolveController(ref);
	if (self == nullptr) return 0.0;
...
double storage_controller_cancel(uint64_t ref)
{
	firebase::storage::Controller* self = resolveController(ref);
	if (self == nullptr) return 0.0;
...
double storage_controller_is_paused(uint64_t ref)
{
	firebase::storage::Controller* self = resolveController(ref);
	if (self == nullptr) return 0.0;
...
double storage_controller_bytes_transferred(uint64_t ref)
{
	firebase::storage::Controller* self = resolveController(ref);
	if (self == nullptr) return 0.0;
...
double storage_controller_total_byte_count(uint64_t ref)
{
	firebase::storage::Controller* self = resolveController(ref);
	if (self == nullptr) return 0.0;
...
uint64_t storage_controller_get_reference(uint64_t ref)
{
	firebase::storage::Controller* self = resolveController(ref);
	if (self == nullptr) return 0;
	return registerStorageReference(self->GetReference());
...
double storage_list_result_release(uint64_t ref)
{
	firebase::storage::StorageListResult* self = resolveListResult(ref);
	if (self == nullptr) return 0.0;
	return unregisterFirebaseValue(gm_fb_ref_id(ref), g_storage_list_result_map) ? 1.0 : 0.0;
}

double storage_list_result_is_valid(uint64_t ref)
{
	firebase::storage::StorageListResult* self = resolveListResult(ref);
	if (self == nullptr) return 0.0;
	return self->is_valid() ? 1.0 : 0.0;
...
double storage_list_result_item_count(uint64_t ref)
{
	firebase::storage::StorageListResult* self = resolveListResult(ref);
	if (self == nullptr) return 0.0;
...
uint64_t storage_list_result_item_at(uint64_t ref, double index)
{
	firebase::storage::StorageListResult* self = resolveListResult(ref);
	if (self == nullptr) return 0;
...
double storage_list_result_prefix_count(uint64_t ref)
{
	firebase::storage::StorageListResult* self = resolveListResult(ref);
	if (self == nullptr) return 0.0;
...
uint64_t storage_list_result_prefix_at(uint64_t ref, double index)
{
	firebase::storage::StorageListResult* self = resolveListResult(ref);
	if (self == nullptr) return 0;
...
std::string storage_list_result_next_page_token(uint64_t ref)
{
	firebase::storage::StorageListResult* self = resolveListResult(ref);
	if (self == nullptr) return std::string();
