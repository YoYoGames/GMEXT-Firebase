...
copyFirebaseCoreJson() {
    local destination_dir="$1"

    mkdir -p "$destination_dir"
    cp -f "$FIREBASE_JSON_SOURCE" "$destination_dir/google-services.json"

    echo "[GMFirebaseCore] Desktop Firebase config copied:"
    echo "[GMFirebaseCore]   $destination_dir/google-services.json"
}

packageLinuxFirebaseCoreJson() {
    ...
    echo "[GMFirebaseCore]   $project_zip -> google-services.json"
    echo "[GMFirebaseCore]   $project_zip -> assets/google-services.json (fallback)"
}
...
case "$YYPLATFORM_name" in
    macOS|Mac|MacOS|OSX)
        ...
        while IFS= read -r -d '' macos_dir; do
            copyFirebaseCoreJson "$macos_dir"
    ...
    *)
        ...
        if [ -n "$YYprojectName" ]; then
            while IFS= read -r -d '' exe; do
                copyFirebaseCoreJson "$(dirname "$exe")"
                COPIED_NEXT_TO_EXE=1
            done < <(
                find "$YYoutputFolder" \
                    -type f \
...
if [ "$COPIED_NEXT_TO_EXE" -eq 0 ]; then
    echo "[GMFirebaseCore] Exact desktop executable was not found; using YYoutputFolder."
fi

copyFirebaseCoreJson "$YYoutputFolder"
