...
// GMFirebaseCore_ExtGen_JNI_OnLoad for Android. This file then owns the real
// JNI_OnLoad, captures the JavaVM, and forwards to ExtGen's loader.
#ifdef JNI_OnLoad
#undef JNI_OnLoad
#endif

extern "C" jint GMFirebaseCore_ExtGen_JNI_OnLoad(JavaVM* vm, void* reserved);
#endif
...
namespace
{
    ...
    JNIEnv* getAndroidJniEnv(std::string& error_message)
    {
        ...
        {
            error_message =
                "Android JavaVM was not captured by GMFirebaseCore JNI_OnLoad";
            return nullptr;
        }
...
// generated symbol to GMFirebaseCore_ExtGen_JNI_OnLoad. This wrapper becomes the
...
extern "C" jint JNI_OnLoad(JavaVM* vm, void* reserved)
{
    ...
    const jint result = GMFirebaseCore_ExtGen_JNI_OnLoad(vm, reserved);
    if (result == JNI_ERR)
...
{
    LOG_INFO("firebase_app_initialize: called");
#if defined(__ANDROID__)
    if (g_firebase_app != nullptr)
    {
        LOG_INFO("firebase_app_initialize: reusing existing g_firebase_app=%p", (void*)g_firebase_app);
        return 1.0;
    }
    ...
    if (firebase::App* existing = firebase::App::GetInstance())
    {
        ...
        LOG_INFO("firebase_app_initialize: reused firebase::App::GetInstance()=%p", (void*)g_firebase_app);
        return 1.0;
    }
    ...
    if (g_firebase_app == nullptr)
    {
        LOG_ERROR("firebase_app_initialize: failed to create Android firebase::App (%s)",
            android_error.empty() ? "no details" : android_error.c_str());
    ...
    LOG_INFO("firebase_app_initialize: created Android firebase::App=%p", (void*)g_firebase_app);
    return 1.0;
#else
	firebase::App* app = getFirebaseApp();
	if (app == nullptr)
	{
		LOG_ERROR("firebase_app_initialize: getFirebaseApp() returned null");
		setFirebaseLastError(-1, "firebase_app_initialize: failed to create firebase::App");
		return 0.0;
	}
	LOG_INFO("firebase_app_initialize: firebase::App=%p ready", (void*)app);
	return 1.0;
#endif
}
